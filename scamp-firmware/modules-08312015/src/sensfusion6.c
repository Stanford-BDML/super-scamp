...
float sensfusion6GetAccXYMag2WRTGravity(const float ax, const float ay, const float az)
{
  float gx, gy, gz; // estimated gravity direction
  ...
  float AdotG = ax*gx + ay*gy + az*gz;
  float flatx = ax - AdotG*gx;
  float flaty = ay - AdotG*gy;
  float flatz = az - AdotG*gz;

  float flat_mag2 = flatx*flatx + flaty*flaty + flatz*flatz;
